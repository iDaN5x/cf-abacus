...
const runQuery = function *(query, variables) {
  ...
  return yield yieldable(graphql(graphSchema, query, undefined, variables));
};
...
routes.post(
  '/v1/metering/aggregated/usage/graph',
  throttle(function *(req) {
    debug(
      'Retrieving rated usage using graphql query %s', req.body.query);
    ...
    const v = req.body.variables;

    // Run the given GraphQL query and return the result
    const doc = yield runQuery(q, v);
